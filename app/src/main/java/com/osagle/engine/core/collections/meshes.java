...
public class meshes extends Thread implements iCallBack {
    ...
    public meshes(osagle pOsagle, long pThreadSleep) {
        mOsagle = pOsagle;
    ...
    public void run() {
        try {
            debug.log("OSAGLE.meshes", "add");
            while(true) { this.doing(); sleep(mThreadSleep); }
        }
        catch(Exception e) {
            debug.log("OSAGLE.meshes", "run.Exception : "+e.toString());
        }
    }
    public void todo(JSONObject pJSON) {
        debug.log("OSAGLE.meshes", "todo : " + pJSON.toString());
        ...
        try  {
            ...
            debug.log("OSAGLE.meshes", "todo.Exception : "+e.toString());
        }
    }
    public void doing() {
        debug.log("OSAGLE.meshes", "doing");
        if (mMapTodo.size() > 0) {
    ...
    public void create(String pKey, JSONObject pJSON) {
        debug.log("OSAGLE.meshes", "create : " + pJSON.toString());

        setup(null, pJSON);
    }
    public void setup(Object pObject, JSONObject pJSON) {
        debug.log("OSAGLE.meshes", "setup");
    }
}