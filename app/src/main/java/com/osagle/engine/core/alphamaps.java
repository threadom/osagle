...
public class alphamaps extends Thread implements iCallBack {
    ...
    public void run() {
        try {
            ...
            debug.log("OSAGLE.alphamaps", "run.Exception : "+e.toString());
        }
    }
    public void todo(JSONObject pJSON) {
        ...
        try  {
            ...
            debug.log("OSAGLE.alphamaps", "todo.Exception : "+e.toString());
        }
    }
    public void doing() {
        debug.log("OSAGLE.alphamaps", "doing");
        if (mMapTodo.size() > 0) {
    ...
    public void setup(Object pObject, JSONObject pJSON) {
        debug.log("OSAGLE.alphamaps", "setup");
    }
}