...
public class lights extends Thread implements iCallBack {
    ...
    public lights(osagle pOsagle, long pThreadSleep) {
        mOsagle = pOsagle;
    ...
    public void run() {
        try {
            debug.log("OSAGLE.lights", "add");
            while(true) { this.doing(); sleep(mThreadSleep); }
        }
        catch(Exception e) {
            debug.log("OSAGLE.lights", "run.Exception : "+e.toString());
        }
    }
    public void todo(JSONObject pJSON) {
        debug.log("OSAGLE.lights", "todo : " + pJSON.toString());
        ...
        try  {
            ...
            debug.log("OSAGLE.lights", "todo.Exception : "+e.toString());
        }
    }
    public void doing() {
        debug.log("OSAGLE.lights", "doing");
        if (mMapTodo.size() > 0) {
    ...
    public void create(String pKey, JSONObject pJSON) {
        debug.log("OSAGLE.lights", "create : " + pJSON.toString());

        setup(null, pJSON);
    }
    public void setup(Object pObject, JSONObject pJSON) {
        debug.log("OSAGLE.lights", "setup");
    }
}